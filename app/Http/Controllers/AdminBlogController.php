...
class AdminBlogController extends Controller
{
    ...
    {
        return Inertia::render('admin/Blog', ['posts' => BlogPost::query()->latest()->get()]);
    }
    ...
    private function validated(Request $request, ?BlogPost $post = null): array
    {
        $data = $request->validate([
            'title' => ['required', 'string', 'max:190'], 'slug' => ['nullable', 'string', 'max:190'], 'excerpt' => ['nullable', 'string', 'max:500'],
            'content' => ['required', 'string'], 'image' => ['nullable', 'url', 'max:500'], 'published_at' => ['nullable', 'date'], 'is_published' => ['required', 'boolean'],
        ]);
        $slug = trim((string) ($data['slug'] ?? ''));
        $data['slug'] = Str::slug($slug !== '' ? $slug : $data['title']);
        $query = BlogPost::query()->where('slug', $data['slug']);
        if ($post) $query->where('id', '!=', $post->getKey());
        if ($query->exists()) $data['slug'] .= '-' . Str::lower(Str::random(5));
